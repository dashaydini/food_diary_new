...
import '../../place/models/place_visit.dart';
...
class _AddVisitScreenState extends State<AddVisitScreen> {
  ...
  void initState() {
    ...
    if (visit != null) {
      ...
      final savedPrices = List<double>.from(visit.prices);

      final foodItems = visit.itemName.split('\n');
      final drinkItems = visit.notes.split('\n');

      foodPrices = List<double>.generate(
        foodItems.length,
        (i) => i < savedPrices.length ? savedPrices[i] : 0,
      );

      drinkPrices = List<double>.generate(
        drinkItems.length,
        (i) {
          final priceIndex = foodItems.length + i;
          return priceIndex < savedPrices.length ? savedPrices[priceIndex] : 0;
        },
      );
    }
  ...
  Future<void> save() async {
    ...
    try {
      ...
      if (widget.existingVisit != null) {
        ...
        final newVisit = PlaceVisit(
          itemName: dishController.text.trim(),
  ...
  Widget _priceRatingButton({
    ...
    return InkWell(
      onTap: readOnly
          ...
          : () {
              setState(() {
                priceRating = priceRating == value ? 0 : value;
                hasChanges = true;
              });
            },
  ...
  Widget _itemSection({
    ...
    while (prices.length < items.length) {
      prices.add(0);
    }
          ...
          children: [
            ...
            if (!readOnly)
              IconButton(
                ...
                onPressed: () {
                  final currentItems = controller.text.split('\n');

                  if (currentItems.length == 1 &&
                      currentItems.first.trim().isEmpty) {
                    currentItems[0] = '';
                  } else {
                    currentItems.add('');
                  }
                  ...
                  controller.text = currentItems.join('\n');
          ...
          return Padding(
            ...
            child: Row(
              children: [
                Expanded(
                  child: TextFormField(
                    key: ValueKey("$title-item-$index-$item"),
                    initialValue: item,
                    readOnly: readOnly,
                ...
                SizedBox(
                  width: 90,
                  child: TextFormField(
                    key: ValueKey("$title-price-$index-${prices[index]}"),
                    initialValue:
                        prices[index] == 0 ? '' : prices[index].toString(),
                    readOnly: readOnly,
        ...
        children: [
          ...
          Row(
            children: List.generate(
              5,
              (index) => Icon(
                index < currentValue.round() ? Icons.star : Icons.star_border,
          ...
          body: ListView(
            ...
            children: [
              ...
              Wrap(
                spacing: 8,
                runSpacing: 8,
                children: (readOnly ? tags : allTags).map(
                  (tag) {
                    return FilterChip(
                      label: Text(tag),
